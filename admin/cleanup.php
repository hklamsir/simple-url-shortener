...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['start_cleanup'])) {
    ...
    if ($conn) {
        try {
            ...
            // 2. 查找並刪除超過 100 天未使用的非自訂短網址
            // 修正：將 su.created_at 加入 GROUP BY 子句中以解決 'Unknown column' 錯誤
            $find_query = "
                SELECT su.url_id FROM short_urls su
                LEFT JOIN access_logs al ON su.url_id = al.url_id
                WHERE su.is_custom = 0
                GROUP BY su.url_id, su.created_at
                HAVING MAX(al.access_time) < NOW() - INTERVAL 100 DAY 
