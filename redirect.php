...
if ($result->num_rows > 0) {
    ...
    // 安全修復 #16：重定向前驗證目標 URL 協議，防止 javascript:/data: 等非 HTTP 協議
    if (!preg_match('/^https?:\/\//i', $original_url)) {
        $stmt->close();
        $conn->close();
        header("Location: " . BASE_URL . "?error=invalid_url");
        exit();
    }
    ...
    // 安全修復 #15：改用 302 臨時重定向 + Cache-Control
    // 短網址服務需要可撤銷性：目標 URL 可能變更或被刪除
    header("Cache-Control: max-age=300, must-revalidate"); // 5 分鐘快取，到期需重新驗證
